...
const { getDailyNumerologyPrediction } = require('./astrology/numerlogyPrediction');
...
async function getDailyBlessings(userId) {
    try {
        ...
        const numerologyPrediction = await getDailyNumerologyPrediction({
            date: userData.birthDate,
            time: userData.birthTime,
            location: { latitude: userData.birthPlace.latitude, longitude: userData.birthPlace.longitude }
        });

        const dailyBlessings = {
            luckyMantra,
            auspiciousTime,
            luckyNumber: numerologyPrediction.luckyNumber,
            luckyColor: numerologyPrediction.luckyColor,
            date: todayIST,
            userId: userId
        };
